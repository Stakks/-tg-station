/mob/living/carbon/dwarf/say_quote(input, spans)
	if(!input)
		return "says, \"...\""	//not the best solution, but it will stop a large number of runtimes. The cause is somewhere in the Tcomms code
	verb_say = dna.species.say_mod
	if(src.health_status.verbal_slur)
		input = attach_spans(input, spans)
		return "slurs, \"[input]\""

	return ..()

/mob/living/carbon/dwarf/treat_message(message)
	if(dna)
...
/mob/living/carbon/dwarf/get_spans()
	return ..() | dna.mutations_get_spans()

/mob/living/carbon/dwarf/GetVoice()
...
/mob/living/carbon/dwarf/IsVocal()
	if(mind)
		return !mind.miming
	return 1

/mob/living/carbon/dwarf/proc/SetSpecialVoice(var/new_voice)
	if(new_voice)
		special_voice = new_voice
	return

/mob/living/carbon/dwarf/proc/UnsetSpecialVoice()
	special_voice = ""
	return

/mob/living/carbon/dwarf/proc/GetSpecialVoice()
	return special_voice

/mob/living/carbon/dwarf/binarycheck()
	if(ears)
		var/obj/item/device/radio/headset/dongle = ears
		if(!istype(dongle)) return 0
		if(dongle.translate_binary) return 1

/mob/living/carbon/dwarf/radio(message, message_mode, list/spans)
	. = ..()
	if(. != 0)
		return .
...
/mob/living/carbon/dwarf/get_alt_name()
	if(name != GetVoice())
		return " (as [get_id_name("Unknown")])"

/mob/living/carbon/dwarf/proc/forcesay(list/append) //this proc is at the bottom of the file because quote fuckery makes notepad++ cri
	if(stat == CONSCIOUS)
		if(client)

...
/mob/living/carbon/dwarf/Life()
	set invisibility = 0
	set background = BACKGROUND_ENABLED

//	if(stat == DEAD)
//		plasma_revive()	//Plasmaman resurrection - Disabled on dwarves for now, but I do plan to add plasmadwarves later on. - DerFlammenwerfer
...
/mob/living/carbon/dwarf/calculate_affecting_pressure(var/pressure)
	..()
	var/pressure_difference = abs( pressure - ONE_ATMOSPHERE )

	var/pressure_adjustment_coefficient = 25	//Determins how much the clothing you are wearing protects you in percent. - Gave dwarves a pressure resistance, as they are usually subterranean
...
/mob/living/carbon/dwarf/handle_disabilities()
	..()
	//Eyes
	...
	if (getBrainLoss() >= 60 && stat != DEAD)
		if (prob(3))
			switch(pick(1,2,3,4))
				if(1)
					say(pick("IM DRUNK YOU DONT HAVE AN EXCUSE", "without booze dwarf dont work", "MAYORS A COMDOM", "a few moments ago, [pick("", "that bloody night creature")] [pick("joerge", "george", "gorge", "gdoruge")] [pick("mellens", "melons", "mwrlins")] attacked me. It is [pick("terrifying", "terrible", "horrifying", "horrific", "awful")]", "can u give me [pick("red spinal","nobility","directions to Icyspires the Cavern of Reckoning")]?", "ded fort", "Bi is THE BEST OF BOTH WORLDS>", "I WANNA PET TEH monkeyS", "stop grifing me!!!!", "SOTP IT#"))
				if(2)
					say(pick("BRAAAAAAAAAP","sodding goblins!", "it's for the best", "it was inevitable", "death is all around us...", "waaaaaagh!!!", "red wonz go fasta", "FOR TEH MOUNTED HOMOS", "lol2cat", "dem humans man, dem humans", "SPESS MAHREENS", "hwee did eet fhor khayosss", "lifelike texture ;_;", "luv can bloooom", "PACKETS!!!"))
				if(3)
					say(pick("That's not your authority.", "he does it [pick("fo fwee", "without compemsatiun")]!!", "traps [pick("are", "aren't")] gay!", "THE DICK MACKES IT BETTA!", "10BUX", "[pick("tbh", "TBH", "TEA BEE AITCH")]", "fcuking pubbies  sware 2 GOD", "PRAISE KEK!!!!!!!SHADILAY!!", "ONE LINE A DAY", "I'M GOIN TA FLAGELLATE YA WITH MAH FUCKIN BEARD"))
				if(4)
					emote(pick("drool", "ree", "clap"))


/mob/living/carbon/dwarf/handle_mutations_and_radiation()
	if(dna)
		if(dna.species.handle_mutations_and_radiation(src))
			..()

/mob/living/carbon/dwarf/breathe()
	if(dna)
		dna.species.breathe(src)

	return

/mob/living/carbon/dwarf/handle_environment(datum/gas_mixture/environment)
	if(dna)
...
///FIRE CODE
/mob/living/carbon/dwarf/handle_fire()
	if(dna)
...
/mob/living/carbon/dwarf/IgniteMob()
	if(dna)
		dna.species.IgniteMob(src)
	else
		..()

/mob/living/carbon/dwarf/ExtinguishMob()
	if(dna)
...
/mob/living/carbon/dwarf/proc/stabilize_temperature_from_calories()
...
/mob/living/carbon/dwarf/proc/get_heat_protection_flags(temperature) //Temperature is the temperature you're being exposed to.
...
/mob/living/carbon/dwarf/proc/get_heat_protection(temperature) //Temperature is the temperature you're being exposed to.
...
/mob/living/carbon/dwarf/proc/get_cold_protection_flags(temperature)
	var/thermal_protection_flags = 0
	//Handle normal clothing
...
/mob/living/carbon/dwarf/proc/get_cold_protection(temperature)
...
/mob/living/carbon/dwarf/handle_chemicals_in_body()
	..()
	if(dna)
...
/mob/living/carbon/dwarf/handle_vision()
	if(machine)
...
/mob/living/carbon/dwarf/handle_hud_icons()
	if(dna)
		dna.species.handle_hud_icons(src)

/mob/living/carbon/dwarf/handle_random_events()
...
/mob/living/carbon/dwarf/handle_changeling()
	if(mind)
		if(mind.changeling)
...
/mob/living/carbon/dwarf/has_smoke_protection()
	if(wear_mask)
	...
	if(head)
		if(head.flags & BLOCK_GAS_SMOKE_EFFECT)
			. = 1
	return .
/mob/living/carbon/dwarf/proc/handle_embedded_objects()
...
/mob/living/carbon/dwarf/handle_heart()
	if(!heart_attack)
		return
	else
		losebreath += 5
...
//mob/living/carbon/dwarf/proc/plasma_revive() - Disabled on dwarves for now for implementation later.
...
//mob/living/carbon/dwarf/proc/plasmaman_resurrection(temperature)
